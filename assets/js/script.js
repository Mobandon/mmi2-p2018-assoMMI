$(document).ready(function () {
    ...
    $.getJSON('membres.json', function (data) {
      ...
      var items = data.membres.map(function (item) {
        ...
        if (item.tel != "") {outp += "<p><i class='fa fa-phone'></i>" + item.tel + "</p>"}
        if (item.email != "") {outp += "<p><i class='fa fa-envelope'></i><a href='mailto:" + item.email +"'>" + item.email + "</p></a>"}
        if (item.poste != "") {outp += "<p><i class='fa fa-user'></i>" + item.poste + "</p>"}
        return outp + "</div>";
      });
